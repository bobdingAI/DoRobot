#!/bin/bash
#
# Unified Piper Robot + Anything-U-Arm Launcher
#
...
export ARM_LEADER_PORT="${ARM_LEADER_PORT:-/dev/ttyUSB0}"
export SERVO_TYPE="${SERVO_TYPE:-zhonglin}"
...
cleanup_stale_sockets() {
    ...
    rm -f "$SOCKET_IMAGE" "$SOCKET_JOINT"
    pkill -f "arm_normal_uarm_v1/main.py" 2>/dev/null || true
...
start_dora() {
    ...
    cd "$DORA_DIR"
    dora run dora_teleoperate_uarm_dataflow.yml &
    DORA_PID=$!
    sleep 2
    DORA_GRAPH_NAME=$(dora list 2>/dev/null | grep -oP 'dora_teleoperate_uarm_dataflow[^\s]*' | head -1) || true
    cd "$PROJECT_ROOT"
}

start_cli() {
    log_step "Starting CLI..."
    local repo_id="${REPO_ID:-piper-uarm-test}"
    local single_task="${SINGLE_TASK:-test piper arm with uarm leader.}"
    
    python "$PROJECT_ROOT/operating_platform/core/main.py" \
        --robot.type=piper_uarm \
