...
def save_device_config(video_devices: list, serial_devices: list, output_path: str = None, set_chmod: bool = False):
    ...
    Default device mapping (plug in this order):
    - /dev/video0 = Top camera
    - /dev/video2 = Wrist camera
    - /dev/ttyACM0 = Leader arm
    - /dev/ttyACM1 = Follower arm

    Note: We use fixed default paths for simplicity. User should plug
    devices in the correct order to get consistent assignments.
    """
    if output_path is None:
        output_path = os.path.expanduser("~/.dorobot_device.conf")

    # Build config content with fixed default paths
    lines = [
        ...
        "# Top camera (plug first)",
        'CAMERA_TOP_PATH="/dev/video0"',
        "",
        "# Wrist camera (plug second)",
        'CAMERA_WRIST_PATH="/dev/video2"',
        "",
        "# === Arm Configuration (Single Arm System) ===",
        "# Leader arm (plug first)",
        'ARM_LEADER_PORT="/dev/ttyACM0"',
        "",
        "# Follower arm (plug second)",
        'ARM_FOLLOWER_PORT="/dev/ttyACM1"',
        "",
    ]

    # Show detected devices as comments for reference
    lines.append("# === Detected Devices (for reference) ===")

    if video_devices:
        for dev in video_devices:
            model = dev.get("model", "Camera")
            lines.append(f"# Video: {dev['path']} - {model}")
    else:
    ...
    if serial_devices:
        for dev in serial_devices:
            ...
            lines.append(f"# Serial: {dev['path']} - {model}")
    else:
