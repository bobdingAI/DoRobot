...
### Persistent USB Port Configuration
Device indices (0, 2, ttyACM0) can change between episodes or when cables are reconnected.
For stable port assignments, use persistent paths based on USB topology:

```bash
# Detect your device paths
python scripts/detect_usb_ports.py --yaml
```

Then update `dora_teleoperate_dataflow.yml`:
```yaml
# Camera - use /dev/v4l/by-path/...
CAPTURE_PATH: "/dev/v4l/by-path/platform-xhci-hcd.0-usb-0:1:1.0-video-index0"

# Serial - use /dev/serial/by-path/... or /dev/serial/by-id/...
